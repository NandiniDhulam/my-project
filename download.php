...
if ($infoResult && $infoResult->num_rows > 0) {
    ...
    $logoPath = htmlspecialchars($businessInfo['Logo']);  // Assuming logo path is stored in the 'Logo' column
}
...
if ($imagePath) {
    ...
    $newWidth = 1200;  // New width for the banner
    $newHeight = 800;  // New height for the banner
    ...
    // Set text color (white, red, green for different parts)
    ...
    // Load font
    $fontPath = 'C:\xampp\htdocs\my project\font\arial.ttf'; // Update this path as necessary
    if (!file_exists($fontPath)) {
        die('Font file does not exist: ' . $fontPath);
    }

    // Adding business information to the image
